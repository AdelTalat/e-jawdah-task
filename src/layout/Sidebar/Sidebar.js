    ...
    return menuItems[navType].map((item) => {
      return (
        <li className="aside-wrapper__nav__item" key={item.id}>
          <Link to={item.link} className="text-white text-decoration-none opacity-75 d-block py-10 px-30">
            {item.iconName ? <i className={`bi ${item.iconName} mx-10 fs-5`}></i> : null} <FormattedMessage id={`${navType}.${item.name}`} defaultMessage={item.name} />
          </Link>
        </li>
      );
    });
    ...
    <aside className="aside-wrapper h-100">
      ...
      <div className="aside-wrapper__user px-30">
        <div className="d-flex justify-content-between mb-10">
          ...
          <select className="form-select" value={context.locale} onChange={context.selectLanguage}>
        ...
        <p className="aside-wrapper__user__info text-white fs-5 pb-5 d-flex justify-content-between">
          <span className="aside-wrapper__user__name">
            <i className="bi bi-person"></i> <FormattedMessage id="sidebar.user" defaultMessage="أحمد خالد الغامدى القحطاني"/>
          </span>
        ...
        <nav className="aside-wrapper__nav">
          ...
          <ul className="aside-wrapper__nav__items list-unstyled px-0 mt-20">
            <p className="opacity-75 text-white px-30 fw-bold">
              <FormattedMessage id="sidebar.electronicServices" defaultMessage="الخدمات الالكترونية"/>
            </p>
          ...
          <ul className="aside-wrapper__nav__items list-unstyled px-0 mt-20">
            <p className="opacity-75 text-white px-30 fw-bold">
              <FormattedMessage id="sidebar.otherLinks" defaultMessage="روابط اخري"/>
            </p>
            {navItems("otherLinks")}
          </ul>
        </nav>
